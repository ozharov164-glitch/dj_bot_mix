  ...
  switch (status) {
    case "QUEUED": {
      ...
      return pos != null && pos > 0
        ? `Позиция в очереди ≈ ${pos}. Можно закрыть приложение — пришлём уведомление.`
        : "Задача в очереди. Можно закрыть приложение — пришлём уведомление.";
    }
    case "RUNNING":
    ...
    case "FAILED":
      return (
        ...
        "Не удалось обработать аудио. Можно попробовать снова."
      );
    default: {
      const _exhaustive: never = status;
      return _exhaustive;
